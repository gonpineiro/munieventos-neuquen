...
$(document).ready(function () {
  ...
  //verifica el campo requiere inscripción para mostrar requiere pre-inscripción

  $("#evento-req_incripcion").change(function () {
    if ($("#i1:checked").val() != 0) {
      $("#preInscricion").show();
...
function verExpositoresModal(link) {
  ...
  $.ajax({
    ...
    $("#modalEvento").modal("show").find(".modal-body").html(data);
    $("#modalEvento")
      .find(".modal-header")
      .html(
        "<h3> Lista de expositores </h3><button type='button' class='close' data-dismiss='modal' aria-label='Close'><span aria-hidden='true'>&times;</span></button>"
      );
  });
}
...
function verRespuestasModal(unaUrl) {
  ...
  $.ajax({
    ...
    $("#modalRespuestas").modal("show").find(".modal-body").html(data);
    $("#modalRespuestas").find(".modal-header").html("<h3>Ver respuestas</h3>");
  });
}
...
function editPresentacionModal(link) {
  ...
  $.ajax({
    ...
    $("#modalEvento")
      .find(".modal-header")
      .html(
        "<h3> Editar presentación </h3><button type='button' class='close' data-dismiss='modal' aria-label='Close'><span aria-hidden='true'>&times;</span></button>"
      );
  });
}
...
function agregarPresentacionModal(link) {
  ...
  $.ajax({
    ...
    $("#modalEvento")
      .find(".modal-header")
      .html(
        "<h3> Cargar presentación </h3><button type='button' class='close' data-dismiss='modal' aria-label='Close'><span aria-hidden='true'>&times;</span></button>"
      );
  });
}
...
function verPresentacionModal(link) {
  ...
  $.ajax({
    ...
    $("#modalEvento")
      .find(".modal-header")
      .html(
        "<h3> Información de la Presentación </h3><button type='button' class='close' data-dismiss='modal' aria-label='Close'><span aria-hidden='true'>&times;</span></button>"
      );
  });
}
...
function generarOpcionesNombreCorto(nombreEvento) {
  ...
  $("#automaticSlug").append(
    '<div class="col-12"><input type="radio" id="otro" name="shortName" value=""> <label for="otro">Otro: </label></div>'
  );
}

function generarSlug(nombreEvento) {
  var slug = nombreEvento
    .toLowerCase()
    .replace(/[^\w ]+/g, "") //reemplaza caracteres alfanumericos
    .replace(/ +/g, "-"); //

  var html =
    '<div class="col-12"> <input type="radio" id="opc1" name="shortName" value="' +
    slug +
    '"> ' +
    '<label for="opc1"> ' +
    slug +
...
function generarInicialesYear(nombreEvento) {
  ...
  var inicialesYear = nombreEvento.match(/\b(\w)/g).join("");
  inicialesYear += year;
  var html =
    '<div class="col-12"><input type="radio" id="opc2" name="shortName" value="' +
    inicialesYear +
    '"> ' +
    '<label for="opc2"> ' +
    inicialesYear +
...
function generarCortoYear(nombreEvento) {
  ...
  //
  var html =
    '<div class="col-12">  <input type="radio" id="opc3" name="shortName" value="' +
    cortoYear +
    '"> ' +
    '<label for="opc3"> ' +
    cortoYear +
...
function editarPerfilModal(unaUrl, titulo) {
  ...
  $.ajax({
    ...
    $("#profileModal").modal("show").find(".modal-body").html(data);
    $("#profileModal")
      .find(".modal-header")
      .html("<h3> " + titulo + " </h3>");
  });
...
function agregarPreguntaModal(unaUrl, titulo) {
  ...
  $.ajax({
    ...
    $("#modalPregunta").modal("show").find(".modal-body").html(data);
    $("#modalPregunta")
      .find(".modal-header")
      .html("<h3> " + titulo + " </h3>");
  });
...
function visualizarQrModal(unaUrl, titulo) {
  ...
  $.ajax({
    ...
    $("#QRModal").modal("show").find(".modal-body").html(data);
    $("#QRModal")
      .find(".modal-header")
      .html("<h3> " + titulo + " </h3>");
  });
...
function abrirContactoModal(usuario, datosUsuario) {
  ...
  var opcionesDisplay = new Array("tabla", "broma"),
    opcionDisplayRandom =
      opcionesDisplay[Math.floor(Math.random() * opcionesDisplay.length)];

  alert(opcionDisplayRandom);

  $("#aboutUsModal").modal("show");
  $("#aboutUsModal").find(".modal-title").html(usuario);
  $("#aboutUsModal")
    .find(".modal-body")
...
function cargarContenidoModalAboutUs(opcion, arrayContacto) {
  ...
  switch (opcion) {
    ...
    default:
      ...
      content +=
        '<table class="table table-hover col-md-8 col-sm-12"> <tbody class="text-white">';

      $.each(arrayContacto[0]["contacto"], function (indice) {
        content +=
          '<tr> <th scope="col">' +
          arrayContacto[0]["contacto"][indice].titulo +
          "</th> <td> " +
          arrayContacto[0]["contacto"][indice].content +
          "</td> </th> </tr>";
      });
